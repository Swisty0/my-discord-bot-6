...
const PREFIX = '!';
const STAFF_LOG_ID = process.env.STAFF_LOG_ID;
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'select_ticket_category') {
      ...
      await interaction.editReply({ content: `Oluşturuldu: ${channel}` });
    }
    ...
    if (interaction.isButton()) {
      ...
      else if (interaction.customId === 'open_giveaway_modal') {
      ...
      else if (interaction.customId === 'open_embed_modal') {
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'staff_modal') {
        ...
        if (logChannel) {
          await logChannel.send({ embeds: [new EmbedBuilder().setTitle('Yeni Başvuru').setDescription(`Ad: ${interaction.fields.getTextInputValue('name')}\nYaş: ${interaction.fields.getTextInputValue('yas')}\nNeden: ${interaction.fields.getTextInputValue('why')}`)] });
        }
        await interaction.editReply({ content: 'Başvurun iletildi!' });
      }
      else if (interaction.customId === 'giveaway_setup_modal') {
        await interaction.deferReply({ ephemeral: true });
        ...
        await interaction.editReply({ content: '✅ Çekiliş başlatıldı!' });
      }
        ...
        await interaction.editReply({ content: '✅ Embed başarıyla oluşturuldu ve gönderildi!' });
      }
    }
  } catch (error) {
